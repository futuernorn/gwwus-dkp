var aowowURL = "db.vanillagaming.org";
...
var Icon = {
	...
	setTexture: function (d, c, b) {
		...
		if (b.indexOf("?") != -1) {
			...
			a.backgroundImage = "url(http://"+aowowURL+"/" + Icon.sizes[c] + "/" + b.toLowerCase() + ".jpg)"
		}
...
var Tooltip = {
	...
	setIcon: function (a) {
		Tooltip.prepare();
		if (a) {
			Tooltip.icon.style.backgroundImage = "url(http://"+aowowURL+"/images/icons/medium/" + a.toLowerCase() + ".jpg)";
...
function () {
	...
	function b(W, S, X, Q, T) {
		 ...
		 * "=" + S + "&power" + R;
		 */
		var P = "http://"+aowowURL+"/ajax.php?" + p[W][1] + "=" + S + "&version=0&power" + R;
		g_ajaxIshRequest(P)
	}
	function N(R, S) {
		if (J._fixTooltip) {
	...
	_link.href = "http://getweirdwith.us/css/power.css";
	_script.setAttribute("src","http://"+aowowURL+"/templates/wowhead/js/locale_enus.js");
